...
const touch_string_literal_handler = require('/handler/touch_string_literal_handler')
const touch_template_literal_handler = require('/handler/touch_template_literal_handler')
    ...
    enter(node) {
        ...
        if (node.type === "TemplateElement") {
          if (node.value.raw.includes("__proto__")) {
              ...
              this.break();
          }else if(node.value.raw.includes("touch")){
            addFinding("touch_template_literal", node.value.raw);
              this.break();
          }
        }
        ...
        if (node.type === "Literal" && typeof node.value === "string") {
          if (node.value.includes("__proto__")) {
            ...
            this.break();
          }else if(node.value.includes("touch")){
            addFinding("touch_string_literal", node.value);
            this.break();
          }
        }
    }
    ...
    for (const finding of findings) {
        if(finding.type == 'array_with_proto'){
          ...
          if(finding.type == "template_literal"){
            ...
            string_literal_handler(filePath,code,finding.value,finding.type)
          }else if(finding.type == "touch_string_literal"){
            touch_string_literal_handler(filePath,code,finding.value,finding.type)
          }
          else if(finding.type == "touch_template_literal"){
            touch_template_literal_handler(filePath,code,finding.value)
          }
          else{
            regular_mutator(filePath,code)
          }
